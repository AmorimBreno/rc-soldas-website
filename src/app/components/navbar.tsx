...
export function Navbar() {
  ...
  return (
    <nav className="fixed left-0 top-0 z-50 flex w-full items-center justify-between bg-black/20 p-5 px-8 font-mont uppercase text-white backdrop-blur-md transition-all duration-700">
      <RiMenu2Line
      ...
      {isMenuOpen && (
        <>
          ...
          />
          <div
            className={`absolute left-0 top-0 z-30 flex h-screen w-4/5 flex-col gap-8 bg-black p-12 transition-all duration-700 ${fade ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}
          >
            <IoClose
